...
namespace HierarchicalJPS.HPA
{
    public class Pathfinder
    {
        ...
        public static LinkedList<Edge> FindPathJPS(Node start, Node goal)
        {
            ...
            var track = new BackTrack();

            gScore[start.pos] = 0;
            var startEdge = new Edge(null, start, EdgeType.INTRA, 0);
            track.AddEdge(start.pos, startEdge, null);
            ...
            while (pq.Count > 0)
            {
                ...
                {
                    //Rebuild path and return it
                    var path = RebuildPathJPS(track, current.end.pos);
                    return path;
                }
                ...
                foreach (var neighbour in neighbours)
                {
                    ...
                    if (jumpNode != null)
                    {
                        track.AddEdge(jumpNode.end.pos, jumpNode, current.end.pos);
